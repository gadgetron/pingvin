...
namespace Gadgetron {
    ...
    void ImageSortGadget::process(Core::InputChannel<mrd::AnyImage> &input, Core::OutputChannel &output) {
        ...
        auto getImageIndex = [&](const auto& i){
          auto &header = i.head;
          if (parameters_.sorting_dimension.size() == 0) {
              return uint32_t(-1);
            } else if (parameters_.sorting_dimension.compare("average") == 0) {
              return header.average.value_or(0);
            } else if (parameters_.sorting_dimension.compare("slice") == 0) {
              return header.slice.value_or(0);
            } else if (parameters_.sorting_dimension.compare("contrast") == 0) {
              return header.contrast.value_or(0);
            } else if (parameters_.sorting_dimension.compare("phase") == 0) {
              return header.phase.value_or(0);
            } else if (parameters_.sorting_dimension.compare("repetition") == 0) {
              return header.repetition.value_or(0);
            } else if (parameters_.sorting_dimension.compare("set") == 0) {
